...
class Cryptopotato:
    ...
    def getAuthor(self):
        self.Author = self.soup.find(class_="entry-user").find("a").text

    def getTitle(self):
        self.Title = self.soup.find(class_="page-title").text.replace('\n', '')

    def getDate(self):
        Date = self.soup.find(class_='last-modified-timestamp').text
        Date = Date.upper()
