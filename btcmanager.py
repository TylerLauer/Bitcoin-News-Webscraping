...
class Btcmanager:
    ...
    def getAuthor(self):
        self.Author = self.soup.find(class_="post post-image entry-content").find("a").text

    def getTitle(self):
        self.Title = self.soup.find(class_="post post-image entry-content").find("h1").text.strip("\n")

    def getDate(self):
        Date = self.soup.find(class_="date").text
        Date = Date.upper()
