...
class Zycrypto:
    ...
    def getAuthor(self):
        self.Author = self.soup.find(class_="td-post-author-name").find("a").text

    def getTitle(self):
        self.Title = self.soup.find(class_="td-post-title").find("h1").text

    def getDate(self):
        Date = self.soup.find(class_='td-post-date').text
        Date = Date.upper()
